...
%The MATLAB function y=filter(h,1,x) computes the convolution y[n] of the
...
%a(1)=>1
y4 = filter(num,den,x); % <=> moving-averaging filter (y1)


% ve do thi xep chong y3[n], y4[n] de test
figure(5);
hold on
plot(n,y3(1:L),'b--',n, y4(1:L),'b*');
xlabel('Thoi gian');
ylabel('Bien do');
legend('y3[n]', 'y4[n]');
title('cach3 vs. cach 4');
